<template>
  <div class="series">
    ...
    <div class="series__slider-list">
      <h3 class="text-3xl font-bold mb-6 ml-7">Danh sách Series</h3>
...
const fetchSlideShowMovies = async () => {
  try {
    const response = await apiServices.getSeries({ per_page: 5 })
...
const fetchSeries = async (params) => {
  try {
    const response = await apiServices.getSeries(params)
